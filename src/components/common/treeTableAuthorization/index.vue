<template>
  <el-table :data="formatData" :row-style="showRow" empty-text="  " v-bind="$attrs" height="100%" style="width: 100%">
    ...
    <el-table-column v-else v-for="(column, index) in columns" :key="column.value" :label="column.text" :width="column.width">
      <template slot-scope="scope">
        ...
        <el-checkbox-group  v-if="Array.isArray(scope.row[column.value])" v-model="scope.row.selectchecked" @change="handleSonChecked(scope.$index, scope.row,scope.row[column.option])">
          <el-checkbox v-for="(interset) in scope.row[column.value]" :label="interset.id" :key="interset.id">{{interset.description}}</el-checkbox>
        </el-checkbox-group>
        <el-checkbox  v-else-if="scope.row.type===1"  :indeterminate="scope.row.isIndeterminate" v-model="scope.row.checkAll" @change="handleSonCheckAll(scope.$index, scope.row,scope.row[column.option])" >{{scope.row[column.value]}}</el-checkbox>
        <span v-else>{{scope.row[column.value]}}</span>
        <el-checkbox  v-if ="scope.row[column.act]"   :indeterminate="scope.row.isIndeterminate" v-model="scope.row.checkAll" @change="handleCheckAll(scope.$index, scope.row,column.option)" >{{scope.row[column.act]}}</el-checkbox>
      </template>
    </el-table-column>
    <slot></slot>
  </el-table>
